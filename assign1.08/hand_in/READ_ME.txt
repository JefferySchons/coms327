...
1.05 	make sure to have numlock on, when going to a new level of dungion no monster will
	move, it will be just like as if you started a new game.
	'c' is used as well as esc to close monster list (the 'c' being used was added
	becouse i kept hitting 'c' to close the list)

1.06	it shows the fow correctly, might show rooms through wals if they are to close.
	when remaking file make sure the .exe file is gone otherwise it has problems
	making the .o files (i have no clue why this is a problem)

1.08	eddit the monster_desc.txt to add monsters, max turn is still there,
	to increase max turn eddit line 1305 (varialbe max_turns) 
	of dungion_make_dungion.cpp