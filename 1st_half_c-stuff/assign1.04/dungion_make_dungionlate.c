...
int main(int argc, char *argv[]){
	...
	int num_monsters=5; //defult //average of 1 per room (minus pc room)
	...
	for(i=1; i<num_monsters; i++)
	{
		if(creature_array[i].alive!=0) //if monster is alive it is placed
		{
			...
			while(tries<32)//I try to place it 32 times before giving up on this monster and it is DOD (dead on Delivery)
			{
  			...
  			if(dungion_map_input[rand_x][rand_y]!='+')
  			{
...
int run_game(char monster_map_input[map_x_size][map_y_size], char dungion_map_input[map_x_size][map_y_size], int dungion_hardness_map_input[map_x_size][map_y_size], char pathfinding_map_input[map_x_size][map_y_size], struct room room_array_input[number_of_rooms], int map_x_size, int map_y_size, int number_of_rooms, int num_monsters, struct creature creature_array[num_monsters])
{
	...
	printf("%c", 'b');
	while(creature_array[0].alive==0);// && turn<=100) //the turn limmit is temperary for quick testing
	{
		//move pc
		//print map
		//k=print_map(dungion_map, map_x_size, map_y_size, number_of_rooms, monster_map);
						...
						if(erratic==1)
						{
							...
							else
							{
								...
								else
								{
									...
									if(0<line_of_sight_map_input[creature_array[i].x_pos][creature_array[i].y_pos] && pathfinding_map_input[creature_array[i].x_pos][creature_array[i].y_pos]<61)
									{
										pc_spoted=0;
									}
									...
									if(pc_spoted==1)
									{
										//random wondering pc not spoted
										if(tunneler==1)
										{
											if(random_direction==0) //pos x, down
											{
												if(dungion_map_input[creature_array[i].x_pos+1][creature_array[i].y_pos]!='+' && dungion_hardness_map_input[creature_array[i].x_pos+1][creature_array[i].y_pos]!=255)
												{
													creature_array[i].x_pos=creature_array[i].x_pos+1;
													creature_array[i].y_pos=creature_array[i].y_pos;
											...
											{
												if(dungion_map_input[creature_array[i].x_pos][creature_array[i].y_pos+1]!='+' && dungion_hardness_map_input[creature_array[i].x_pos][creature_array[i].y_pos+1]!=255)
												{
													creature_array[i].x_pos=creature_array[i].x_pos;
													creature_array[i].y_pos=creature_array[i].y_pos+1;
									...
									{
										//not telapathic (line of sight)
										if(intellignet==1) //n i
										{
											if(tunneler==1) //n i t
											{
												//will tunnel using pathfinding to PC; if can see PC
												//they are waisting tunneling
											}
											else //n i n
											{
												//will go stright to pc if they see the PC
											}
										}
										else
										{
											if(tunneler==1) //n n t
											{
												//random wonderer that can tunnel if pc not spoted
											}
											else //n n n //random wonderer
											{
												//
												//if pc not spoted
												//random wonderer
