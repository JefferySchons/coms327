  ...
  */
  ...
  /*
	for(j=0; j<map_y_size; j++)
	{
		...
		{
			(*dungion_map_input)[j][k]=dungion_map[j][k];
			(*dungion_hardness_map_input)[j][k]=dungion_hardness_map[j][k];
	  }
	}
	...
	*/
	...
	for(j=0; j<map_y_size; j++)
	{
		for(k=0; k<map_x_size; k++)
		{
			...
			{
				printf("%c", dungion_map_input[k][j]);
			}
	  }
